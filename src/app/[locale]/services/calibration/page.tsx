...
export default function CalibrationPage() {
  const t = useTranslations('calibrationPage');
      ...
      <section className="relative bg-navy py-20 lg:py-28 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="w-16 h-16 rounded-2xl bg-gold/20 flex items-center justify-center mx-auto mb-6">
            <Gauge className="w-8 h-8 text-gold" />
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollSection>
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold text-navy mb-4">{t('equipmentTitle')}</h2>
              <p className="text-gray-500 max-w-2xl mx-auto">{t('equipmentDesc')}</p>
            </div>
            <div className="grid md:grid-cols-2 gap-8">
              {['aed', 'ecg', 'sterilizer', 'diagnostic'].map((key) => (
                <div
                  key={key}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollSection className="flex flex-col lg:flex-row-reverse gap-12 items-center">
            ...
            <div className="w-full lg:w-3/5">
              ...
              <ul className="space-y-3">
                {['process1', 'process2', 'process3', 'process4'].map((key) => (
                  <li key={key} className="flex items-start gap-3">
                    <CheckCircle className="w-5 h-5 text-gold shrink-0 mt-0.5" />
                    <span className="text-gray-600">{t(key)}</span>
                  </li>
